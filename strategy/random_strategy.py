"""strategy/random_strategy.py — Random selection baseline"""
...
class FedAvgWithRandom(FedAvg):
    ...
    def configure_fit(self, server_round, parameters, client_manager):
        ...
        selected = random.sample(all_clients, min(self.k_select, len(all_clients)))
        print(f"\n[Random] Round {server_round}")

        config = self.on_fit_config_fn(server_round) if self.on_fit_config_fn else {}
        config["recent_dropout_rate"] = 0.0
        return [(c, FitIns(parameters, config)) for c in selected]
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        reward   = -float(np.mean(he_norms)) - 0.3 * dropout_count
